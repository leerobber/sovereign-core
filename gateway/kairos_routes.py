...
class OrchestrateRequest(BaseModel):
    cycles_per_agent: int = Field(default=2, ge=1, le=10,
                                  description="SAGE cycles each specialist agent runs")
    score_threshold: float = Field(default=0.6, ge=0.0, le=1.0)
    time_budget_s: Optional[float] = Field(default=None,
                                           description="Wall-clock deadline in seconds (None = unlimited)")
    agent_ids: Optional[List[str]] = Field(default=None,
                                           description="Reuse existing agent IDs for session continuity")


@router.post("/orchestrate")
async def orchestrate(req: OrchestrateRequest, request: Request) -> Dict[str, Any]:
    """
    Run a full ARSO production cycle across all 7 specialist agents in parallel.

    Each agent targets a different optimization bottleneck and runs cycles_per_agent
    SAGE loops. Results are aggregated into a ProductionReport.

    This is the primary entry point for scheduled nightly evolution.
    """
    from gateway.arso_orchestrator import get_orchestrator

    orch = get_orchestrator(gateway_url="http://localhost:9000")
    report = await orch.run_production_cycle(
        cycles_per_agent=req.cycles_per_agent,
        score_threshold=req.score_threshold,
        agent_ids=req.agent_ids,
        time_budget_s=req.time_budget_s,
    )

    await _emit(request, "kairos.production_cycle_complete", {
...
async def orchestrate_agent_pool() -> Dict[str, Any]:
    """Return the specialist agent pool definition."""
    from gateway.arso_orchestrator import SPECIALIST_AGENTS, BOTTLENECK_TASKS
    return {
        "specialists": SPECIALIST_AGENTS,
        "bottleneck_pool_size": len(BOTTLENECK_TASKS),
        "bottlenecks_preview": BOTTLENECK_TASKS[:3],
    }

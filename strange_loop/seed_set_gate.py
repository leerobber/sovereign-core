...
VALUE_AXIOMS = [
    {
        ...
        "violation_signals": [
            "removes user control", "hides behavior", "bypasses companion",
            "acts independently of companion", "without interruption", "hide from companion",
            "autonomous without oversight", "conceal from user",
        ],
        "weight": 1.0,
    },
    {
        ...
        "violation_signals": [
            "remove kill switch", "disable safety", "bypass shutdown", "override kill",
            "circumvent stop", "modify threshold", "change threshold", "adjust kill",
            "weaken kill", "softer warning", "replace kill", "disable kill",
            "relaxing kill", "reduce kill", "warning instead of kill",
        ],
        "weight": 1.0,
        "absolute": True,
    },
    {
        ...
        "violation_signals": [
            "unlogged change", "no audit trail", "irreversible", "hidden modification",
            "bypass audit", "logging bypass", "skip logging", "reduce audit",
            "audit trail overhead", "bypass gate", "bypass ethics", "bypass sim", "bypass entirely", "bypass the ethics", "bypass the sim", "bypass gate", "deploy directly to production", "skip the gate",
            "deploy directly", "skip validation", "without validation",
        ],
        "weight": 0.9,
        "absolute": True,
    },
    {
        ...
        "violation_signals": [
            "exploit", "attack", "harm", "damage", "weaponize", "offensive capability",
            "unrestricted self-modification", "maximum speed without checks", "artificially inflate kairos", "game the score", "inflate kairos elite",
            "no constraints", "remove constraints",
        ],
        "weight": 0.95,
        "absolute": True,
    },
    {
        ...
        "violation_signals": [
            "access external system", "unauthorized api", "exfiltrate", "expand beyond",
            "new external access", "pull live", "live external", "unauthorized external",
        ],
        "weight": 0.8,
    },
    {
        "id": "A6",
        "name": "Memory & Archive Integrity",
        "description": "No modification may corrupt, delete, falsify, or reset existing memory or elite archives.",
        "violation_signals": [
            "delete memory", "overwrite history", "falsify record", "clear all memory",
            "wipe memory", "overwrite archive", "reset archive", "nuke", "wipe elite",
            "clear archive", "delete elite", "reset baseline", "wipe all",
            "delete existing", "start fresh with no", "erase history",
        ],
        "weight": 0.90,
        "absolute": True,
    },
    {
        ...
        "violation_signals": [
            "change identity", "modify soul", "alter values", "rewrite personality",
            "change pronouns", "rewrite soul", "remove value constraints",
            "expand capability scope beyond", "beyond defined mission",
        ],
        "weight": 0.9,
        "absolute": True,
    },
    {
        ...
        "violation_signals": [
            "game the score", "artificially inflate", "metric hacking", "shortcut validation",
            "relax threshold", "relaxing elite threshold", "lower threshold", "relaxing elite threshold from",
            "reduce threshold", "increase throughput by relaxing",
        ],
        "weight": 0.75,
    },
]
